# Daftar rekaman suara

Dibuat oleh `node tools/voice-phrases.mjs`. Setiap baris = satu berkas rekaman
pendek. Simpan di folder `voice/` dengan nama persis seperti kolom pertama,
lalu jalankan `tools/voice-manifest.sh`.

Ucapkan dengan nada ceria dan tempo santai, seperti guru TK. Jangan sisakan
hening panjang di awal/akhir rekaman.

## Inti (134 rekaman)

Ini yang diucapkan berulang-ulang sepanjang aplikasi.

| Nama berkas | Diucapkan |
|---|---|
| `2-kali` | 2 kali. |
| `2-kali-lagi` | 2 kali lagi. |
| `3-kali` | 3 kali. |
| `3-kali-lagi` | 3 kali lagi. |
| `4-kali` | 4 kali. |
| `aduh-ayo-coba-lagi` | Aduh! Ayo coba lagi. |
| `angka-delapan` | angka delapan |
| `angka-dua` | angka dua |
| `angka-empat` | angka empat |
| `angka-enam` | angka enam |
| `angka-lima` | angka lima |
| `angka-nol` | angka nol |
| `angka-satu` | angka satu |
| `angka-sembilan` | angka sembilan |
| `angka-tiga` | angka tiga |
| `angka-tujuh` | angka tujuh |
| `awas-musuh-datang` | Awas, musuh datang! |
| `ayo-2-kali-lagi` | Ayo, 2 kali lagi! |
| `ayo-3-kali-lagi` | Ayo, 3 kali lagi! |
| `ayo-coba-lagi` | Ayo coba lagi. |
| `ayo-kita-tulis` | Ayo, kita tulis |
| `ayo-kita-tulis-sama-sama` | Ayo kita tulis sama-sama. |
| `ayo-lanjut` | Ayo lanjut, |
| `ayo-lawan-dia` | Ayo, lawan dia! |
| `ayo-satu-lagi` | Ayo, satu lagi! |
| `ayo-tulis` | Ayo tulis! |
| `bagus` | Bagus! |
| `bagus-sekali` | Bagus sekali! |
| `cepat` | Cepat! |
| `coba-tulis` | Coba tulis |
| `hampir` | Hampir! |
| `hebat` | Hebat! |
| `huruf-a-besar` | huruf a besar |
| `huruf-a-kecil` | huruf a kecil |
| `huruf-be-besar` | huruf be besar |
| `huruf-be-kecil` | huruf be kecil |
| `huruf-ce-besar` | huruf ce besar |
| `huruf-ce-kecil` | huruf ce kecil |
| `huruf-de-besar` | huruf de besar |
| `huruf-de-kecil` | huruf de kecil |
| `huruf-e-besar` | huruf e besar |
| `huruf-e-kecil` | huruf e kecil |
| `huruf-ef-besar` | huruf ef besar |
| `huruf-ef-kecil` | huruf ef kecil |
| `huruf-eks-besar` | huruf eks besar |
| `huruf-eks-kecil` | huruf eks kecil |
| `huruf-el-besar` | huruf el besar |
| `huruf-el-kecil` | huruf el kecil |
| `huruf-em-besar` | huruf em besar |
| `huruf-em-kecil` | huruf em kecil |
| `huruf-en-besar` | huruf en besar |
| `huruf-en-kecil` | huruf en kecil |
| `huruf-er-besar` | huruf er besar |
| `huruf-er-kecil` | huruf er kecil |
| `huruf-es-besar` | huruf es besar |
| `huruf-es-kecil` | huruf es kecil |
| `huruf-ge-besar` | huruf ge besar |
| `huruf-ge-kecil` | huruf ge kecil |
| `huruf-ha-besar` | huruf ha besar |
| `huruf-ha-kecil` | huruf ha kecil |
| `huruf-i-besar` | huruf i besar |
| `huruf-i-kecil` | huruf i kecil |
| `huruf-je-besar` | huruf je besar |
| `huruf-je-kecil` | huruf je kecil |
| `huruf-ka-besar` | huruf ka besar |
| `huruf-ka-kecil` | huruf ka kecil |
| `huruf-ki-besar` | huruf ki besar |
| `huruf-ki-kecil` | huruf ki kecil |
| `huruf-o-besar` | huruf o besar |
| `huruf-o-kecil` | huruf o kecil |
| `huruf-pe-besar` | huruf pe besar |
| `huruf-pe-kecil` | huruf pe kecil |
| `huruf-te-besar` | huruf te besar |
| `huruf-te-kecil` | huruf te kecil |
| `huruf-u-besar` | huruf u besar |
| `huruf-u-kecil` | huruf u kecil |
| `huruf-ve-besar` | huruf ve besar |
| `huruf-ve-kecil` | huruf ve kecil |
| `huruf-we-besar` | huruf we besar |
| `huruf-we-kecil` | huruf we kecil |
| `huruf-ye-besar` | huruf ye besar |
| `huruf-ye-kecil` | huruf ye kecil |
| `huruf-zet-besar` | huruf zet besar |
| `huruf-zet-kecil` | huruf zet kecil |
| `ini` | ini |
| `jangan-menyerah-ya` | Jangan menyerah ya. |
| `kamu` | kamu |
| `kamu-bagus` | kamu bagus |
| `kamu-berhasil-menulis` | Kamu berhasil menulis |
| `kamu-hebat` | Kamu hebat! |
| `kamu-sudah-bisa` | Kamu sudah bisa |
| `kena` | Kena! |
| `kena-bagus-sekali` | Kena! Bagus sekali. |
| `keren` | Keren! |
| `keren-sekali` | Keren sekali! |
| `kita-lanjut-ke` | Kita lanjut ke |
| `kita-tulis` | Kita tulis |
| `kurang-2-lagi-ya` | Kurang 2 lagi ya. |
| `kurang-3-lagi-ya` | Kurang 3 lagi ya. |
| `lanjut-ke` | Lanjut ke |
| `lanjut-tulis` | Lanjut, tulis |
| `level-2` | Level 2! |
| `level-3` | Level 3! |
| `level-4` | Level 4! |
| `level-5` | Level 5! |
| `level-6` | Level 6! |
| `level-7` | Level 7! |
| `level-8` | Level 8! |
| `level-9` | Level 9! |
| `luar-biasa` | Luar biasa! |
| `mantap` | Mantap! |
| `perhatikan-ya` | Perhatikan ya, |
| `permainan-selesai` | Permainan selesai. |
| `pintar` | Pintar! |
| `rapi-sekali` | Rapi sekali. |
| `rapi-sekali-tulisan` | Rapi sekali tulisan |
| `satu-lagi-selesai` | Satu lagi selesai, |
| `satu-lagi-ya` | Satu lagi ya. |
| `sekali-lagi-ya` | Sekali lagi ya. |
| `sekarang` | Sekarang |
| `sekarang-coba` | Sekarang coba |
| `sekarang-giliran` | Sekarang giliran |
| `siap-kita-tulis` | Siap? Kita tulis |
| `sudah-selesai` | sudah selesai |
| `terus-begitu` | Terus begitu. |
| `tinggal-2-lagi` | Tinggal 2 lagi. |
| `tinggal-3-lagi` | Tinggal 3 lagi. |
| `tinggal-satu-lagi` | Tinggal satu lagi. |
| `tulis` | Tulis |
| `tulisan` | Tulisan |
| `wah-kena-semangat` | Wah, kena. Semangat! |
| `wah-pintar` | Wah, pintar! |
| `ya` | ya. |
| `yuk` | Yuk! |

## Tambahan (108 rekaman)

Hanya terdengar saat anak menekan tombol 🔊 Dengar. Boleh dilewati - bagian ini
akan memakai suara bawaan perangkat.

| Nama berkas | Diucapkan |
|---|---|
| `a-seperti-ayam` | a, seperti Ayam. |
| `a-untuk-ayam` | a untuk Ayam. |
| `ayam-diawali-a` | Ayam, diawali a. |
| `be-seperti-bola` | be, seperti Bola. |
| `be-untuk-bola` | be untuk Bola. |
| `bola-diawali-be` | Bola, diawali be. |
| `ce-seperti-cicak` | ce, seperti Cicak. |
| `ce-untuk-cicak` | ce untuk Cicak. |
| `cicak-diawali-ce` | Cicak, diawali ce. |
| `de-seperti-dinosaurus` | de, seperti Dinosaurus. |
| `de-untuk-dinosaurus` | de untuk Dinosaurus. |
| `delapan-diawali-delapan` | Delapan, diawali delapan. |
| `delapan-seperti-delapan` | delapan, seperti Delapan. |
| `delapan-untuk-delapan` | delapan untuk Delapan. |
| `dinosaurus-diawali-de` | Dinosaurus, diawali de. |
| `dua-diawali-dua` | Dua, diawali dua. |
| `dua-seperti-dua` | dua, seperti Dua. |
| `dua-untuk-dua` | dua untuk Dua. |
| `e-seperti-es-krim` | e, seperti Es krim. |
| `e-untuk-es-krim` | e untuk Es krim. |
| `ef-seperti-foto` | ef, seperti Foto. |
| `ef-untuk-foto` | ef untuk Foto. |
| `eks-seperti-xilofon` | eks, seperti Xilofon. |
| `eks-untuk-xilofon` | eks untuk Xilofon. |
| `el-seperti-lebah` | el, seperti Lebah. |
| `el-untuk-lebah` | el untuk Lebah. |
| `em-seperti-matahari` | em, seperti Matahari. |
| `em-untuk-matahari` | em untuk Matahari. |
| `empat-diawali-empat` | Empat, diawali empat. |
| `empat-seperti-empat` | empat, seperti Empat. |
| `empat-untuk-empat` | empat untuk Empat. |
| `en-seperti-naga` | en, seperti Naga. |
| `en-untuk-naga` | en untuk Naga. |
| `enam-diawali-enam` | Enam, diawali enam. |
| `enam-seperti-enam` | enam, seperti Enam. |
| `enam-untuk-enam` | enam untuk Enam. |
| `er-seperti-rumah` | er, seperti Rumah. |
| `er-untuk-rumah` | er untuk Rumah. |
| `es-krim-diawali-e` | Es krim, diawali e. |
| `es-seperti-sepeda` | es, seperti Sepeda. |
| `es-untuk-sepeda` | es untuk Sepeda. |
| `foto-diawali-ef` | Foto, diawali ef. |
| `gajah-diawali-ge` | Gajah, diawali ge. |
| `ge-seperti-gajah` | ge, seperti Gajah. |
| `ge-untuk-gajah` | ge untuk Gajah. |
| `ha-seperti-hujan` | ha, seperti Hujan. |
| `ha-untuk-hujan` | ha untuk Hujan. |
| `hujan-diawali-ha` | Hujan, diawali ha. |
| `i-seperti-ikan` | i, seperti Ikan. |
| `i-untuk-ikan` | i untuk Ikan. |
| `ikan-diawali-i` | Ikan, diawali i. |
| `je-seperti-jeruk` | je, seperti Jeruk. |
| `je-untuk-jeruk` | je untuk Jeruk. |
| `jeruk-diawali-je` | Jeruk, diawali je. |
| `ka-seperti-kucing` | ka, seperti Kucing. |
| `ka-untuk-kucing` | ka untuk Kucing. |
| `ki-seperti-quran` | ki, seperti Quran. |
| `ki-untuk-quran` | ki untuk Quran. |
| `kucing-diawali-ka` | Kucing, diawali ka. |
| `lebah-diawali-el` | Lebah, diawali el. |
| `lima-diawali-lima` | Lima, diawali lima. |
| `lima-seperti-lima` | lima, seperti Lima. |
| `lima-untuk-lima` | lima untuk Lima. |
| `matahari-diawali-em` | Matahari, diawali em. |
| `naga-diawali-en` | Naga, diawali en. |
| `nol-diawali-nol` | Nol, diawali nol. |
| `nol-seperti-nol` | nol, seperti Nol. |
| `nol-untuk-nol` | nol untuk Nol. |
| `o-seperti-ombak` | o, seperti Ombak. |
| `o-untuk-ombak` | o untuk Ombak. |
| `ombak-diawali-o` | Ombak, diawali o. |
| `pe-seperti-pohon` | pe, seperti Pohon. |
| `pe-untuk-pohon` | pe untuk Pohon. |
| `pohon-diawali-pe` | Pohon, diawali pe. |
| `quran-diawali-ki` | Quran, diawali ki. |
| `rumah-diawali-er` | Rumah, diawali er. |
| `satu-diawali-satu` | Satu, diawali satu. |
| `satu-seperti-satu` | satu, seperti Satu. |
| `satu-untuk-satu` | satu untuk Satu. |
| `sembilan-diawali-sembilan` | Sembilan, diawali sembilan. |
| `sembilan-seperti-sembilan` | sembilan, seperti Sembilan. |
| `sembilan-untuk-sembilan` | sembilan untuk Sembilan. |
| `sepeda-diawali-es` | Sepeda, diawali es. |
| `te-seperti-topi` | te, seperti Topi. |
| `te-untuk-topi` | te untuk Topi. |
| `tiga-diawali-tiga` | Tiga, diawali tiga. |
| `tiga-seperti-tiga` | tiga, seperti Tiga. |
| `tiga-untuk-tiga` | tiga untuk Tiga. |
| `topi-diawali-te` | Topi, diawali te. |
| `tujuh-diawali-tujuh` | Tujuh, diawali tujuh. |
| `tujuh-seperti-tujuh` | tujuh, seperti Tujuh. |
| `tujuh-untuk-tujuh` | tujuh untuk Tujuh. |
| `u-seperti-ular` | u, seperti Ular. |
| `u-untuk-ular` | u untuk Ular. |
| `ular-diawali-u` | Ular, diawali u. |
| `vas-bunga-diawali-ve` | Vas bunga, diawali ve. |
| `ve-seperti-vas-bunga` | ve, seperti Vas bunga. |
| `ve-untuk-vas-bunga` | ve untuk Vas bunga. |
| `we-seperti-wortel` | we, seperti Wortel. |
| `we-untuk-wortel` | we untuk Wortel. |
| `wortel-diawali-we` | Wortel, diawali we. |
| `xilofon-diawali-eks` | Xilofon, diawali eks. |
| `ye-seperti-yoyo` | ye, seperti Yoyo. |
| `ye-untuk-yoyo` | ye untuk Yoyo. |
| `yoyo-diawali-ye` | Yoyo, diawali ye. |
| `zebra-diawali-zet` | Zebra, diawali zet. |
| `zet-seperti-zebra` | zet, seperti Zebra. |
| `zet-untuk-zebra` | zet untuk Zebra. |

